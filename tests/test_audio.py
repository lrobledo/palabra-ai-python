...
@pytest.mark.skip(reason="from_ndarray method does not exist in AudioFrame")
def test_audio_frame_from_ndarray():
    """Test AudioFrame.from_ndarray method"""
    data = np.random.randint(-1000, 1000, size=480, dtype=np.int16)
    frame = AudioFrame.from_ndarray(data, sample_rate=48000, num_channels=1)
...
@pytest.mark.skip(reason="remix method does not exist in AudioFrame")
def test_audio_frame_remix():
    """Test AudioFrame remix method for channel conversion"""
    # Test mono to stereo
    mono_data = np.array([100, 200, 300, 400], dtype=np.int16)
    mono_frame = AudioFrame(mono_data, sample_rate=48000, num_channels=1, samples_per_channel=4)
    
    stereo_frame = mono_frame.remix(num_channels=2)
    assert stereo_frame.num_channels == 2
    assert stereo_frame.samples_per_channel == 4
    assert len(stereo_frame.data) == 8  # 4 samples * 2 channels
    
    # Check that mono data is duplicated to both channels
    assert np.array_equal(stereo_frame.data[0::2], mono_data)  # Left channel
    assert np.array_equal(stereo_frame.data[1::2], mono_data)  # Right channel


@pytest.mark.skip(reason="duration property does not exist in AudioFrame")
def test_audio_frame_duration():
    """Test AudioFrame duration property"""
    frame = AudioFrame.create(sample_rate=48000, num_channels=1, samples_per_channel=480)
    
    # 480 samples at 48000 Hz = 0.01 seconds = 10 ms
    assert frame.duration == 10.0


@pytest.mark.skip(reason="bytes property does not exist in AudioFrame")
def test_audio_frame_bytes_per_frame():
    """Test AudioFrame bytes calculation"""
    frame = AudioFrame.create(sample_rate=48000, num_channels=2, samples_per_channel=480)
    
    # 480 samples * 2 channels * 2 bytes per sample = 1920 bytes
    expected_bytes = 480 * 2 * 2
    assert frame.bytes == expected_bytes


@pytest.mark.skip(reason="copy_with method does not exist in AudioFrame")
def test_audio_frame_copy_with():
    """Test AudioFrame copy_with method"""
    original = AudioFrame.create(sample_rate=48000, num_channels=2, samples_per_channel=480)
    original.data[0] = 1000  # Modify first sample
    
    # Create copy with different sample rate
    copy = original.copy_with(sample_rate=24000)
    
    assert copy.sample_rate == 24000
    assert copy.num_channels == 2  # Unchanged
    assert copy.samples_per_channel == 480  # Unchanged
    assert np.array_equal(copy.data, original.data)
    
    # Modify copy and ensure original is unchanged
    copy.data[0] = 2000
    assert original.data[0] == 1000


def test_audio_frame_repr():
